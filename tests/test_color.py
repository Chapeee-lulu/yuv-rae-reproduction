...
class ColorConversionTests(unittest.TestCase):
    def test_black_and_white_roundtrip_exactly(self) -> None:
        ...
        recovered = yuv444_to_rgb(rgb_to_yuv444(image)).astype(np.uint8)
    ...
    def test_integer_roundtrip_error_is_at_most_one(self) -> None:
        ...
        recovered = yuv444_to_rgb(rgb_to_yuv444(colors)).astype(np.uint8)
    ...
    def test_invalid_shape_is_rejected(self) -> None:
        with self.assertRaises(ValueError):
            rgb_to_yuv444(np.zeros((8, 8), dtype=np.uint8))
